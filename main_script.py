...
# Apply the process_withdrawals function to each row and create a new DataFrame
result_rdd = joined_df.rdd.map(process_withdrawals)
result_df = spark.createDataFrame(result_rdd, ["account_id", "balance_order", "initial_balance", "available_balance", "status", "validation_result"])
result_df.show()
# Write the result to an output file or repository
